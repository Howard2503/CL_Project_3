...
// socket.on("initBoard", ({ boardState: newBoardState, playerInfo: info, playerOnePieces: p1, playerTwoPieces: p2 }) => {
//   playerInfo = info;
...
// socket.on("updateBoardNew", ({ col, row, tile, playerRole, playerOnePieces: p1, playerTwoPieces: p2 }) => {
//   // 更新服务器广播的棋子数据
//   playerOnePieces = p1;
//   playerTwoPieces = p2;
// });
...
function handleDrop(event) {
  ...
  if (index >= 0 && index < boardState.length && playerInfo.role !== "Spectator") {
    ...
    if (isValidMove && !gameReady && !boardState[index]) {
      // 更新本地棋盘状态
...
//   if (playerInfo.role !== "Spectator" && tile) {
//     socket.emit("moveTileNew", { col, row, tile, playerRole: playerInfo.role });
//     generateTile(); // 本地生成新牌
//   }
// }
